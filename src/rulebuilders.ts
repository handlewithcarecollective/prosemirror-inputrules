...
export function markTypeInputRule(
  ...
  return new InputRule(regexp, (state, match, start, end) => {
    ...
    if (!content || !contentIndices || !matchIndices) return null;
    ...
    if (prefix) {
      tr.replace(
        start,
        contentStart,
        new Slice(Fragment.from(state.schema.text(prefix)), 0, 0),
      );
    } else {
    ...
    if (suffix) {
      tr.replace(
        ...
        tr.mapping.map(end),
        new Slice(Fragment.from(state.schema.text(suffix)), 0, 0),
      );
    } else {
